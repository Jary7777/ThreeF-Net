...
class _SimpleSegmentationModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # x0 = F.interpolate(x0, size=input_shape, mode='bilinear', align_corners=False)
        x = F.interpolate(x, size=input_shape, mode='bilinear', align_corners=False)
        return x

#-SSl--
class SSL_SimpleSegmentationModel(nn.Module):
    def __init__(self, backbone, classifier):
        super(SSL_SimpleSegmentationModel, self).__init__()
        self.backbone = backbone
        self.classifier = classifier
    ...
    def forward(self, x):
        input_shape = x.shape[-2:]
        features = self.backbone(x)
        CNN, Trans, raw, x = self.classifier(x, features)
        CNN = F.interpolate(CNN, size=input_shape, mode='bilinear', align_corners=False)
        Trans = F.interpolate(Trans, size=input_shape, mode='bilinear', align_corners=False)
        raw = F.interpolate(raw, size=input_shape, mode='bilinear', align_corners=False)
        x = F.interpolate(x, size=input_shape, mode='bilinear', align_corners=False)


        return CNN, Trans, raw, x
